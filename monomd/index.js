...
function parseList(items) {
    ...
    const parsedItems = items.map(item => {
        ...
        // チェックリストの判定
        const checklistMatch = item.match(/^(\s*)- \[([ x])\]\s(.+)$/s);
        if (checklistMatch) {
    ...
    // HTMLを構築
    function buildListHTML(items, startIndex = 0, parentLevel = -1, parentType = null) {
        let html = '';
        let i = startIndex;
        ...
        function flushCurrentList() {
            if (currentListItems.length > 0 && currentListType) {
                ...
                html += `<${tag}>${currentListItems.join('')}</${tag}>`;
                currentListItems = [];
        ...
        while (i < items.length) {
            ...
            if (item.level === parentLevel + 1) {
                // リストタイプが変わった場合、現在のリストを閉じる
                if (currentListType !== null && item.type !== 'checklist' && item.type !== currentListType) {
                    flushCurrentList();
                }
                ...
                if (item.type === 'checklist') {
                    ...
                    let itemHtml = `<li class="checklist-item"><div class="checklist-content"><input type="checkbox" id="${checkboxId}" ${item.checked ? 'checked' : ''} disabled><label for="${checkboxId}">${parseInline(item.content)}</label></div>`;
                    
                    // 次のアイテムが子要素かチェック
                    if (i + 1 < items.length && items[i + 1].level > item.level) {
                        const childResult = buildListHTML(items, i + 1, item.level, item.type);
                        itemHtml += childResult.html;
                    ...
                    let itemHtml = `<li>${parseInline(item.content)}`;
